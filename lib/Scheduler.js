...
define([], function () {
    ...
    Scheduler.prototype._getMappingOrderOrZero = function(name, mappings){
        for (var i in mappings){
            if(mappings[i].from === name) return mappings[i].order;
        }
        return 0;
    };
            ...
            for(var i=0; i<nextNodeNames.length; i++){
                ...
                if(incomingEdges.length > 1){
                    ...
                    if(existingOutput.length === incomingEdges.length-1){
                        ...
                        for(var i in existingOutput){
                            packets.push({
                                from: existingOutput[i],
                                packet: buffer[existingOutput[i]],
                                order: this._getMappingOrderOrZero(existingOutput[i], mappings)
                            });
                        }
                        ...
                        this._sendPacketToNode(args, nextNodeName, version, id);
                    } else {
